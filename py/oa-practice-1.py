...
"""
Given a string of lowercase English letters s your task is to rearrange its letters by pairing them together according to the following rule. 
First letter with the last one, second letter with the second to last one, etc. 
More formally, if s = c1c2c3…..cn-3*cn -2*cn, then the letters should be arranged in c1cnc2cn-1 if s contains an odd number of letters, 
the middle letter should not be paired with any other letters, 
but placed at the end of the output string instead. 
Example:
s = “abcdef”
solution(s) = “afbecd”
"""
...
"""
Imagine there is a group chat with many users writing messages. 

The content of messages includes text and mentions of other users in the chat. 
Mentions in the group chat are formatted as strings starting 
with the @ character and followed by at least one id separated by commas. 

An id is formatted as a string starting with id and followed by a positive integer from 0  to 999
...
Now imagine you are given two arrays of strings titled members and messages. 
Your task is to calculate the mention statistics from the group chat. 
In other words, count the number of messages that each chat member is mentioned in. 
Chat members mentioned multiple times in a message should be counted only once per message.

Return the mention statistics in an array of strings, where each string follows this format: “[user id] - [mentions count]”. 
The array should be sorted by mention count in descending order, 
or in the case of a tie, lexicographically by userid in ascending order.

It is guaranteed that proper ids will be used for each mention. Additionally, all mentions will be preceded by and followed by a space. 
Unless they are located either at the beginning or end of the message. 
...
def sol2(members, messages):
    ...
    for key in org_dict.keys():
        for id in sorted(org_dict[key]):
            ret += f"[{id}] - [{stats_dict[id]}], "

    return ret.rstrip(', ')


def sol2_regex(members, messages):
    ret, stats_dict = "", {}
    for mem in members:
        stats_dict[mem] = 0
    ...
    pat = "[@]?[id[0-9]+[\, ]]*"
    for msg in messages:
        for m in set([x.lstrip('@').rstrip(',').rstrip() for x in re.findall(pat, msg)]):
            try:
                stats_dict[m] += 1
            except KeyError:
                stats_dict[m] = 1

    stats_dict = dict([x for x in list(stats_dict.items()) if x[0] in members])
    stats_dict = dict(sorted(stats_dict.items(), key=lambda x: x[1], reverse=True))

    stats_grouped = {}
    for a, b in stats_dict.items():
        stats_grouped[b] = [a] if b not in stats_grouped.keys(
        ) else sorted(stats_grouped[b] + [a])

    for k in stats_grouped.keys():
        for r in stats_grouped[k]:
            ret += f"[{r}] - [{k}], "
...
Given an array of integers elements find such t (0 <= t < elements.length) that cyclic t-shift operations 
turns elements into a sorted array [1, 2, . . . , elements.length]. 
...
def sol3(numbers):
    ...
    h1, h2 = numbers[0:numbers.index(
        min(numbers))], numbers[numbers.index(min(numbers)):]
...
messages = ["Hey @id123,id321 review this PR please! @id123 what do you think about this approach?",
            "Hey @id7 nice approach! Great job! @id800 what do you think?",
            "@id323,id121 thx!"]
print(sol2(members, messages))

print(sol2_regex(members, messages))
